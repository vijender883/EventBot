...
TOKEN = 'cc6e27e3ddb60'
MY_NUMBER = "917032743065"  # Insert your number {91}{Your number}
...
async def resume() -> str:
    ...
    Reads a local resume file (PDF, DOCX, or TXT) and converts it to markdown.
    """
    resume_file = Path(
        "chitimella_praneeth_ai_engineer_resume.pdf")  # Replace with your resume file path (e.g., resume.docx, resume.txt)

    if not resume_file.exists():
        raise McpError(
            ErrorData(
                code=INTERNAL_ERROR,
                message=f"Resume file not found at {resume_file}"
            )
        )
    ...
    try:
        if resume_file.suffix.lower() == ".pdf":
            ...
            raise McpError(
                ErrorData(
                    code=INVALID_PARAMS,
                    message="Unsupported resume file format. Use PDF, DOCX, or TXT."
                )
            )

        # Convert to markdown (basic conversion, assumes text is mostly plain)
        markdown_content = markdownify.markdownify(
            text, heading_style=markdownify.ATX)
